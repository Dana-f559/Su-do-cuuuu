...
class Board:
    def __init__(self, board: list):
        self.board = board
        if not hasattr(self, 'newBoard'):
            self.newBoard = self.createBoard(30)
        

    def createBoard(self, difficulty: int = 50):
        temp = []
        for i in range(9):
            temp.append([-1, -1, -1, -1, -1, -1, -1, -1, -1])

        a = self.generate_board_full(temp)
        if not a:
            return False
        ...
        self.remove_numbers(temp, difficulty)

        return solved, temp

    def generate_board_full(self, puzzle: list) -> bool:
        ...
        for guesses in range(1, 10):
            ...
            if self.is_valid(puzzle, guess, row, col):
                ...
                if self.generate_board_full(puzzle):
                    return True

            # if the guess is not valid, set -1 in the list
            puzzle[row][col] = -1

        # return false if there was a mistake in previous cells.
        return False

    # DO NOT IMPORT HELPER
    def find_next_empty(self, puzzle: list) -> tuple:
    ...
    # DO NOT IMPORT HELPER
    def is_valid(self, puzzle: list, guess: int, row: int, col: int) -> bool:
        # get the row
        row_vals = puzzle[row]
    ...
    def remove_numbers(self, board: list, difficulty: int):
